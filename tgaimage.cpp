...
std::ostream& operator<<(std::ostream& ostream, TGAColor color) {
    for (int i = 0; i < 4; ++i)
        ostream << static_cast<int>(color.bgra[i]) << " ";
    return ostream;
}

TGAColor operator*(const TGAColor& color, const float& k) {
    TGAColor ans = color;
    for (int i = 0; i < 3; ++i)
        ans.bgra[i] *= k;
    return ans;
...
TGAColor operator*(const float& k, const TGAColor& color) {
    TGAColor ans = color;
    for (int i = 0; i < 3; ++i)
        ans.bgra[i] *= k;
    return ans;
